...
import { LockClosedIcon, BackArrowIcon, CheckCircleIcon, ExclamationCircleIcon } from './icons/Icons';
...
const stripePromise = loadStripe('pk_test_51HPvU92eZvYxgC9s4zcb3s5c9s2s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s5s00j2s5c9s2');
...
const CheckoutForm: React.FC<{ details: BookingDetails; onConfirm: () => void; isLoggedIn: boolean; }> = ({ details, onConfirm, isLoggedIn }) => {
    ...
    const handleStripeSubmit = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            if (saveCard) {
                ...
                console.log('User opted to save card. Payment Method ID:', paymentMethod.id);
            }
    ...
    const getButtonClass = () => {
        if (paymentStatus === 'successful') return 'bg-green-600 cursor-default';
        if (paymentStatus === 'processing') return 'bg-blue-600 opacity-75 cursor-wait';
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                    <h2 className="text-2xl font-bold mb-4 dark:text-gray-100">Your Booking</h2>
                    <img src={details.hotel.imageUrl} alt={details.hotel.name} className="w-full h-48 object-cover rounded-md mb-4" />
                    <h3 className="text-xl font-semibold dark:text-gray-200">{details.hotel.name}</h3>
                    <p className="text-gray-600 dark:text-gray-400">{details.hotel.city}, {details.hotel.country}</p>
                    <div className="mt-4 pt-4 border-t dark:border-gray-700 space-y-2">
                        ...
                        <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Nights:</span> <span className="font-medium dark:text-gray-200">{details.nights}</span></div>
                    </div>
                    <div className="mt-4 pt-4 border-t dark:border-gray-700">
                        <div className="flex justify-between text-lg font-bold">
                            <span className="dark:text-gray-200">Total Price</span>
                            <span className="dark:text-gray-200">${details.totalPrice}</span>
                        </div>
                    </div>
                </div>
