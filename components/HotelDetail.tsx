    ...
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-xl animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
            ...
            <div className="mb-4 border-b border-gray-200">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <button onClick={() => setActiveTab('nearby')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'nearby' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
                    Nearby Attractions
                    </button>
                    <button onClick={() => setActiveTab('image-editor')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'image-editor' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
                    AI Image Fun
                    </button>
                </nav>
            </div>
            ...
            {activeTab === 'overview' && (
                <div>
                    <h3 className="text-2xl font-bold text-gray-800 mb-4">About this stay</h3>
                    ...
                    {hotel.amenities.map(amenity => (
                        <div key={amenity} className="bg-gray-100 p-3 rounded-md text-center text-gray-700">{amenity}</div>
                    ))}
                    </div>
                </div>
            )}
        ...
        <div className="lg:col-span-1">
            <div className="bg-blue-50 p-6 rounded-lg shadow-md sticky top-24">
                <p className="text-3xl font-bold text-gray-900 mb-2">${hotel.pricePerNight}<span className="text-base font-normal text-gray-600">/night</span></p>
                <div className="space-y-4 mb-6">
                    <div>
                        <label htmlFor="checkin" className="block text-sm font-medium text-gray-700">Check-in</label>
                        <input type="date" id="checkin" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                     <div>
                        <label htmlFor="checkout" className="block text-sm font-medium text-gray-700">Check-out</label>
                        <input type="date" id="checkout" value={checkOut} onChange={e => setCheckOut(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                     <div>
                        <label htmlFor="guests" className="block text-sm font-medium text-gray-700">Guests</label>
                        <input type="number" id="guests" value={guests} min="1" max={hotel.maxGuests} onChange={e => setGuests(Number(e.target.value))} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                </div>

                {totalPrice > 0 && (
                  <div className="bg-white p-4 rounded-md mb-6 border border-blue-200">
